...
export function isAchievementUnlocked(profile: UserProfile, achievementId: string) {
  switch (achievementId) {
    ...
    case "human-calculator":
      return profile.history.some((result) => result.cognitiveGameId === "flash-numbers" && (result.accuracy ?? 0) >= 0.9) || profile.history.some((result) => result.gameType === "memory" && result.perfectRound);
    case "pattern-hunter":
    ...
    case "memory-keeper":
      return domainScore(profile, "learning-knowledge-integration") >= 500;
    case "process-master":
      return profile.history.some((result) => result.cognitiveGameId === "multi-step-execution" && (result.accuracy ?? 0) >= 0.9);
    case "working-memory-beast":
      return domainScore(profile, "working-memory") >= 500;
    case "spatial-wizard":
      return domainScore(profile, "spatial-reasoning") >= 500;
    case "logic-lord":
      return domainScore(profile, "logic-reasoning") >= 500;
    case "focus-monk":
      return domainScore(profile, "attention") >= 500;
    case "verbal-analyst":
      return domainScore(profile, "language-concepts") >= 500;
    case "system-architect":
      return domainScore(profile, "causal-reasoning") >= 500;
    default:
      return false;
  }
}
...
export function getAchievementProgress(profile: UserProfile, achievementId: string) {
  switch (achievementId) {
    ...
    case "memory-keeper":
      return progress(domainScore(profile, "learning-knowledge-integration"), 500);
    case "process-master":
      return isAchievementUnlocked(profile, achievementId) ? 100 : 0;
    case "working-memory-beast":
    ...
    case "logic-lord":
      return progress(domainScore(profile, "logic-reasoning"), 500);
    case "focus-monk":
      return progress(domainScore(profile, "attention"), 500);
    case "verbal-analyst":
      return progress(domainScore(profile, "language-concepts"), 500);
    case "system-architect":
      return progress(domainScore(profile, "causal-reasoning"), 500);
    default:
      return 0;
  }
}
...
export function achievementTargetLabel(achievementId: string) {
  const labels: Record<string, string> = {
    ...
    "human-calculator": "perfect Flash Numbers",
    ...
    "the-outlier": "top 1%",
    "memory-keeper": "500 Learning",
    "process-master": "multi-step perfect",
    "working-memory-beast": "500 Working Memory",
    "spatial-wizard": "500 Spatial",
    "logic-lord": "500 Logic",
    "focus-monk": "500 Attention",
    "verbal-analyst": "500 Language",
    "system-architect": "500 Causal",
  };
